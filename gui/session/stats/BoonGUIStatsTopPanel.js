class BoonGUIStatsTopPanel {
	...
	constructor() {
		...
		this.headerContainer.size = "0 0 100% 18";
		this.rowsContainer.size = "0 16 100% 100%";
		this.scales = new BoonGUIColorScales();
		this.root.sprite = `backcolor: 38 38 38 205`;
	}

	update(playersStates, checkbox) {
		...
		playersStates.forEach(state => {
			...
			this.scales.addValue("champion", state.classCounts.Champion ?? 0);
